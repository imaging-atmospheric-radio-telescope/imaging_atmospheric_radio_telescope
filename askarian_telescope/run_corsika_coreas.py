...
def make_coreas_steering_card(
    ...
    sc = "# CoREAS V1.1 by Tim Huege <tim.huege@kit.edu> with contributions by Marianne Ludwig and Clancy James - parameter file\n"
    sc+= "\n"
    ...
    sc+= "AutomaticTimeBoundaries = 4e-07 ; 0: off, x: automatic boundaries with width x in s\n"
    sc+= "TimeLowerBoundary = -1 ; in s, only if AutomaticTimeBoundaries set to 0\n"
    sc+= "TimeUpperBoundary = 1 ; in s, only if AutomaticTimeBoundaries set to 0\n"
    sc+= "ResolutionReductionScale = 0 ; 0: off, x: decrease time resolution linearly every x cm in radius\n"
    sc+= "\n"
    sc+= "# parameters setting up the simulation functionality:\n"
    sc+= "GroundLevelRefractiveIndex = 1.000292 ; specify refractive index at 0 m asl\n"
    sc+= "\n"
    ...
    sc+= "Comment = \n"
    sc+= "\n"
    sc+= "# event information for your convenience and backwards compatibility with other software, these values are not used as input parameters for the simulation:\n"
    sc+= "\n"
    return sc
...
def make_coreas_antenna_list(imaging_reflector):
    ...
    for i in range(imaging_reflector.huygens_antennas_positions.shape[0]):
        sc += "AntennaPosition = {x:2f}\t{y:2f}\t{z:2f}\t huygens_antenna_{antenna_idx:06d}\n".format(
...
def simulate_air_shower_and_imaging_reflector_response(
    ...
    with tempfile.TemporaryDirectory(prefix='corsika_coreas_') as tmp_dir:
        ...
        with open(tmp_coreas_steering_card_path, 'wt') as fout:
            fout.write(
                make_coreas_steering_card(
                    core_position_on_observation_level_north=core_position_on_observation_level_north,
                    core_position_on_observation_level_west=core_position_on_observation_level_west,
                    observation_level_altitude=observation_level_altitude,
        ...
        with open(out_corsika_o_path, 'w') as corsika_o, open(out_corsika_e_path, 'w') as corsika_e:
...
def simulate_event(
    ...
    for component in ['north_component', 'west_component', 'vertical_component']:
        image_sensor_responses[component] = telescope.simulate_image_sensor_response(
            huygens_matrix=huygens_matrix,
            raw_imaging_reflector_huygens_antenna_responses=raw_imaging_reflector_huygens_antenna_responses,
    ...
    config = {
        ...
        'imaging_reflector': telescope.imaging_reflector_to_dict(imaging_reflector),
    }
...
def event_parameter_distribution(
    ...
    return {
        ...
        'energy': np.random.uniform(low=energy[0], high=energy[1], size=number_events),
        'observation_level_altitude': np.repeat(observation_level_altitude, number_events),
        'core_position_on_observation_level_north': core_north,
        'core_position_on_observation_level_west': core_west,
        'azimuth': np.random.uniform(low=azimuth[0], high=azimuth[1], size=number_events),
