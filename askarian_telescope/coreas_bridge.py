...
def assert_same_time_slice_duration(raw_antenna_time_series, time_slice_duration):
    for antenna in range(raw_antenna_time_series.shape[0]):
        time_slice_duration_this_antenna = np.gradient(raw_antenna_time_series[antenna,:,0])
        valid = np.abs(time_slice_duration_this_antenna - time_slice_duration) < time_slice_duration/100
        assert np.all(valid)
...
def read_all_raw_time_series(path):
    ...
    for time_series_path in all_time_series_paths:
        ...
        time_series[:,COREAS_NORTH_COMPONENT] *= CGS_statVolt_per_cm_to_SI_Volt_per_meter
        time_series[:,COREAS_WEST_COMPONENT] *= CGS_statVolt_per_cm_to_SI_Volt_per_meter
        time_series[:,COREAS_VERTICAL_COMPONENT] *= CGS_statVolt_per_cm_to_SI_Volt_per_meter
