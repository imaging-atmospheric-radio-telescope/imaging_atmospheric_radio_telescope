...
def make_config():
    ...
    c["corsika_coreas_executable_path"] = None
    return c
...
def _simulate_mirror_electric_fields_manual(
    ...
    if corsika_coreas_executable_path is None:
        corsika_coreas_executable_path = (
            corsika.build.get_corsika_executable_path()
        )
    ...
    with tempfile.TemporaryDirectory(prefix="corsika_coreas_") as tmp_dir:
        ...
        with rnw.open(tmp_corsika_steering_card_path, "wt") as fout:
            fout.write(
                corsika.make_steering_card(
                    ...
                    zenith_rad=primary_particle["zenith_rad"],
                    azimuth_rad=primary_particle["azimuth_rad"],
        ...
        with rnw.open(tmp_coreas_steering_card_path, "wt") as fout:
            fout.write(
        ...
        with rnw.open(tmp_coreas_antenna_list_path, "wt") as fout:
            fout.write(
        ...
        with rnw.open(cor_o_path, "w") as cor_o, rnw.open(
            cor_e_path, "w"
        ) as cor_e:
            subprocess.call(
...
def simulate_mirror_electric_fields(
    ...
    if not os.path.exists(probe_dir):
        ...
        _simulate_mirror_electric_fields_manual(
            corsika_coreas_executable_path=airshower_config[
                "corsika_coreas_executable_path"
            ],
            out_dir=probe_dir,
            event_id=airshower_config["event_id"],
            primary_particle=airshower_config["primary_particle"],
            site=site,
