...
parser.add_argument(
    "--telescope_key",
    metavar="TELESCOPE_KEY",
    default="large_size_telescope",
    type=str,
...
def ax_add_telescope_xy(ax, telescope, linewidth, roi):
    ...
    sebplt.ax_add_circle(
        ...
        x=0,
        y=0,
        r=telescope["sensor"]["outer_radius_m"],
        color="black",
        linewidth=linewidth,
        linestyle="-",
        alpha=0.33,
...
close_up_items = {
    "camera": {"range": 1.1 * telescope["sensor"]["outer_radius_m"]},
...
for item in close_up_items:
    ...
    fig.savefig(os.path.join(out_dir, f"telescope_{item:s}_close_up_xy.jpg"))
    sebplt.close(fig)
...
sR = telescope["sensor"]["outer_radius_m"]
...
fig.savefig(os.path.join(out_dir, f"telescope_side_scatch.jpg"))
sebplt.close(fig)
